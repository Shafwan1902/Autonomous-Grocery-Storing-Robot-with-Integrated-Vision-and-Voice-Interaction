...
class ObjectDetection:
    ...
    def visualize_detections(self):
        """
        Visualize detected chairs, humans, and couches with occupancy status and centroids.
        """
        ...
        # Draw couches with centroids
        for couch in self.couches:
            x1, y1, x2, y2 = couch['bbox']
            centroid = couch['centroid']
            color = (0, 255, 255)  # Yellow for couches

            # Draw bounding box
            cv2.rectangle(img, (x1, y1), (x2, y2), color, 2)

            # Put label and centroid
            cv2.putText(img, "Couch", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2)
            cv2.circle(img, centroid, 5, color, -1)  # Draw the centroid as a small circle
